...
public class FilterCommand implements Action {
    ...
    public Object execute() throws Exception {
        transactionOperations.execute((TransactionCallback<Void>) status -> {
            ...
            List<InetAddress> ipAddressList = null;
            try {
                matching = filterDao.isRuleMatching(filterRule);
                if (matching) {
                    nodeMap = filterDao.getNodeMap(filterRule);
                    ipAddressList = filterDao.getActiveIPAddressList(filterRule);
                }
            } catch (Exception e) {
                // pass
            }
            ...
            if (nodeMap != null && !nodeMap.isEmpty()) {
                ...
                System.out.printf("---Matching nodes---\n");
            }
            for (OnmsNode node : nodes) {
                List<String> categoryNames = new ArrayList<>();

                node.getCategories().forEach(category -> categoryNames.add(category.getName()));
                String categories = node.getCategories().isEmpty() ? ""
                        : "categories=" + categoryNames.stream().collect(Collectors.joining(","));
                ...
                System.out.printf("nodeId=%d nodeLabel=%s location=%s %s %s %s \n", node.getId(), node.getLabel(),
                        location, foreignId, foreignSource, categories);

            }
            List<String> ipAddresses = new ArrayList<>();
            if (ipAddressList != null && !ipAddressList.isEmpty()) {
                ipAddressList.stream().forEach(ipAddress -> ipAddresses.add(ipAddress.getHostAddress()));
                System.out.printf("---Matching IP Addresses that are active---\n");
            }
